...
from .initialization_changed import INIT
from .tile_warping import TileWarping
from .tile_update import TileUpdate, PostTileUpdate, FinalTileUpdate, PostTileUpdateNoUp, ConfCal
...
from losses.utils import fill_zeros_2d
...
class HITNet(nn.Module):
    def __init__(self, args):
        ...
        self.conf_cal = ConfCal(32, 1, 16, 2, DispUpsampleBySlantedPlane(2, 2), args)
    ...
    def forward(self, left_img, right_img):
        ...
        conf = self.conf_cal(left_fea_pyramid[4], refined_tile05x)
        # final_disp = final_disp * (conf>0.5)
        # final_disp = fill_zeros_2d(final_disp) 
        # pdb.set_trace()
        ...
        if self.training:
            ...
            # init_disp16_fx = self.prop_disp_upsample64x(init_tile_pyramid[0][:,0,:,:].unsqueeze(1), 0, 0)
            # init_disp8_fx = self.prop_disp_upsample32x(init_tile_pyramid[1][:,0,:,:].unsqueeze(1), 0, 0)
            # init_disp4_fx = self.prop_disp_upsample16x(init_tile_pyramid[2][:,0,:,:].unsqueeze(1), 0, 0)
            # init_disp2_fx = self.prop_disp_upsample8x (init_tile_pyramid[3][:,0,:,:].unsqueeze(1), 0, 0)
            # init_disp1_fx = self.prop_disp_upsample4x (init_tile_pyramid[4][:,0,:,:].unsqueeze(1), 0, 0)
            # init_disp_pyramid = [
            #     init_disp16_fx,
            #     init_disp8_fx,
            #     init_disp4_fx,
            ...
            outputs = {
                ...
                "w_pyramid": w_pyramid,
                # "init_disp_pyramid": init_disp_pyramid,
                "conf": conf
            }
            # pdb.set_trace()
