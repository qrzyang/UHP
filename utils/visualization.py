...
class disp_error_image_func(Function):
    @staticmethod
    def forward(self, D_est_tensor, D_gt_tensor, mask_set = True, abs_thres=3., rel_thres=0.05, dilate_radius=1):
        D_gt_np = D_gt_tensor.detach().cpu().numpy()
        D_est_np = D_est_tensor.detach().cpu().numpy()
        B, H, W = D_gt_np.shape
        # valid mask
        mask = (D_gt_np > 0) & np.array(mask_set)
...
class color_error_image_func(Function):
    @staticmethod
    def forward(self, C_est_tensor, C_gt_tensor, abs_thres=0.3, rel_thres=0.005, dilate_radius=1):
        C_gt_np =torch.norm(C_gt_tensor, 2, 1).detach().cpu().numpy()
        # C_est_np = C_est_tensor.detach().cpu().numpy()
        B, H, W = C_gt_np.shape

        # error in percentage. When error <= 1, the pixel is valid since <= 3px & 5%
        diff = torch.norm(C_est_tensor-C_gt_tensor, 2, 1)
        error = diff.detach().cpu().numpy()
        error = np.minimum(error / abs_thres, (error / C_gt_np) / rel_thres)
        # get colormap
        cols = error_colormap
        # create error image
        error_image = np.zeros([B, H, W, 3], dtype=np.float32)
